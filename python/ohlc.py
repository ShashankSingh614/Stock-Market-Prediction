...
def feature_of_company(company_name):
    ...
    for i, feature in enumerate(FEATURES):
        ...
        if company_name == 'AAPL':
            final_accuracy_scalar, y_pred, date, y_test, MAE, MAPE, MDAPE,predicted_price, change_percent = getrnnmodelaapl(feature)
        elif company_name == 'MSFT':
            final_accuracy_scalar, y_pred, date, y_test, MAE, MAPE, MDAPE,predicted_price, change_percent= getrnnmodelmsft(feature)
        elif company_name == 'GOOG':
            final_accuracy_scalar, y_pred, date, y_test, MAE, MAPE, MDAPE,predicted_price, change_percent = getrnnmodelgoog(feature)
        elif company_name == 'AMZN':
            final_accuracy_scalar, y_pred, date, y_test, MAE, MAPE, MDAPE,predicted_price, change_percent = getrnnmodelamzn(feature)
        elif company_name == 'NVDA':
            final_accuracy_scalar, y_pred, date, y_test, MAE, MAPE, MDAPE,predicted_price, change_percent = getrnnmodelnvda(feature)
        ...
        axes[i].text(0.95, 0.85, f'Predicted Price: {predicted_price}', 
        ...
        axes[i].text(0.95, 0.80, f'Direction: {up_down_text}', 
                    transform=axes[i].transAxes, verticalalignment='top', 
                    horizontalalignment='right', color=text_color, 
                    bbox=dict(boxstyle='round', facecolor='white', alpha=0.7))

        # Add accuracy text to the graph
        axes[i].text(0.95, 0.90, accuracy_text, transform=axes[i].transAxes, 
